...
class Adversarial (nn.Module):
    def __init__(self, trainer, net):
        super().__init__()
        self.env = env #in env.bidder list, the first bidder must be the adversarial one
        self.trainer= trainer #trainer with net that has already been trained (for ex 80 epochs)

    def adv_train(self, n_epochs = 2):
        self.trainer.env.bidders_list[0].is_strategic = True #make the first bidder a strategic one
